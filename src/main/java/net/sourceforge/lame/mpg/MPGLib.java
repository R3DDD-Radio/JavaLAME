...
public class MPGLib {
	...
	private int decode1_headersB_clipchoice(mpstr_tag pmp, byte[] buffer, int bufferPos, int len, float[] pcm_l, int pcm_lPos, float[] pcm_r, int pcm_rPos, MP3Data mp3data, FrameSkip enc, float[] p, int psize, IDecoder decodeMP3_ptr) {
		...
		if(pmp.header_parsed || pmp.fsizeold > 0 || pmp.framesize > 0) {
			...
			if(pmp.fsizeold > 0) /* works for free format and fixed, no overrun, temporal results are < 400.e6 */
				mp3data.bitrate = (int) (8 * (4 + pmp.fsizeold) * mp3data.samplerate / (1.e3 * mp3data.getFrameSize()) + 0.5);
			else if(pmp.framesize > 0)
				mp3data.bitrate = (int) (8 * (4 + pmp.framesize) * mp3data.samplerate / (1.e3 * mp3data.getFrameSize()) + 0.5);
			else
	 ...
	 * channel are allowed.
	 */
	public int hip_decode1_unclipped(mpstr_tag hip, byte[] buffer, int bufferPos, int len, final float pcm_l[], final float pcm_r[]) {
		...
		if(hip != null) {
			...
			return decode1_headersB_clipchoice(hip, buffer, bufferPos, len, pcm_l, 0, pcm_r, 0, mp3data, enc, out, OUTSIZE_UNCLIPPED, dec);
		}
		return 0;
	}
	 ...
	 *         MPEG data.
	 */
	public int hip_decode1_headers(mpstr_tag hip, byte[] buffer, int len, final float[] pcm_l, final float[] pcm_r, MP3Data mp3data, FrameSkip enc) {
		if(hip != null) {
			IDecoder dec = new IDecoder() {
