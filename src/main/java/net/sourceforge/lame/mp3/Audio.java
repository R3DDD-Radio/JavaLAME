...
public class Audio {
	...
	public void initialize(LameGlobalFlags gfp, DataInputStream inputStream, FrameSkip frameSkip) {
		...
		this.inputStream = openSoundStream(gfp, inputStream, frameSkip);
	}
	...
	int read_samples_mp3(LameGlobalFlags gfp, DataInputStream musicin, float mpg123pcm[][]) {
		...
		int out = decodeFromStream(musicin, mpg123pcm[0], mpg123pcm[1], parse.getMp3InputData());
	 ...
	 */
	private int parse_wave_header(LameGlobalFlags gfp, DataInputStream dataInputStream) {
		int format_tag = 0;
		int channels = 0;
		...
		/* file_length = */
		read32BitsHighLow(dataInputStream);
		if(read32BitsHighLow(dataInputStream) != WAV_ID_WAVE)
			return -1;
		
		for(loop_sanity = 0; loop_sanity < 20; ++loop_sanity) {
			int type = read32BitsHighLow(dataInputStream);
			
			if(type == WAV_ID_FMT) {
				subSize = read32BitsLowHigh(dataInputStream);
				if(subSize < 16) {
					return -1;
				}
				
				format_tag = read16BitsLowHigh(dataInputStream);
				subSize -= 2;
				channels = read16BitsLowHigh(dataInputStream);
				subSize -= 2;
				samples_per_sec = read32BitsLowHigh(dataInputStream);
				subSize -= 4;
				/* avg_bytes_per_sec = */
				read32BitsLowHigh(dataInputStream);
				subSize -= 4;
				/* block_align = */
				read16BitsLowHigh(dataInputStream);
				subSize -= 2;
				bits_per_sample = read16BitsLowHigh(dataInputStream);
				subSize -= 2;
				
				/* WAVE_FORMAT_EXTENSIBLE support */
				if((subSize > 9) && (format_tag == WAVE_FORMAT_EXTENSIBLE)) {
					read16BitsLowHigh(dataInputStream); /* cbSize */
					read16BitsLowHigh(dataInputStream); /* ValidBitsPerSample */
					read32BitsLowHigh(dataInputStream); /* ChannelMask */
				...
				if(subSize > 0) {
					try {
						dataInputStream.skipBytes(subSize);
					} catch(IOException e) {
						return -1;
					}
				}
	 ...
	 */
	private int parse_aiff_header(LameGlobalFlags gfp, DataInputStream dataInputStream) {
		...
		int chunkSize = read32BitsHighLow(dataInputStream);
		
		int typeID = read32BitsHighLow(dataInputStream);
		...
		while(chunkSize > 0) {
			...
			if(type == IFF_ID_COMM) {
				...
				subSize = read32BitsHighLow(dataInputStream);
				ckSize = make_even_number_of_bytes_in_length(subSize);
				chunkSize -= ckSize;
				
				aiff_info.numChannels = (short) read16BitsHighLow(dataInputStream);
				ckSize -= 2;
				aiff_info.numSampleFrames = read32BitsHighLow(dataInputStream);
				ckSize -= 4;
				aiff_info.sampleSize = (short) read16BitsHighLow(dataInputStream);
				ckSize -= 2;
				try {
					aiff_info.sampleRate = readIeeeExtendedHighLow(dataInputStream);
				} catch(IOException e1) {
					return -1;
				}
				ckSize -= 10;
				if(typeID == IFF_ID_AIFC) {
					dataType = read32BitsHighLow(dataInputStream);
					ckSize -= 4;
				}
				try {
					dataInputStream.skipBytes((int) ckSize);
				} catch(IOException e) {
					return -1;
				}
				...
				aiff_info.blkAlgn.offset = read32BitsHighLow(dataInputStream);
				ckSize -= 4;
				aiff_info.blkAlgn.blockSize = read32BitsHighLow(dataInputStream);
				ckSize -= 4;
				...
				if(seen_comm_chunk > 0) {
					try {
						dataInputStream.skipBytes(aiff_info.blkAlgn.offset);
					} catch(IOException e) {
						return -1;
					}
					/* We've found the audio data. Read no further! */
		...
		if(seen_comm_chunk != 0 && (seen_ssnd_chunk > 0 || aiff_info.numSampleFrames == 0)) {
			...
			if(pcm_data_pos >= 0) {
				try {
					dataInputStream.skip(pcm_data_pos);
					//dataInputStream.seek(pcm_data_pos);
				} catch(IOException e) {
					if(parse.silent < 10) {
	 ...
	 */
	private SoundFileFormat parse_file_header(LameGlobalFlags gfp, DataInputStream dataInputStream) {
		...
		if(type == WAV_ID_RIFF) {
			...
			int ret = parse_wave_header(gfp, dataInputStream);
			if(ret > 0) {
			...
			int ret = parse_aiff_header(gfp, dataInputStream);
			if(ret > 0) {
				count_samples_carefully = true;
	...
	private DataInputStream openSoundStream(LameGlobalFlags gfp, DataInputStream dataInputStream, FrameSkip frameSkip) {
		...
		if(is_mpeg_file_format(parse.getInputFormat())) {
			if(lame_decode_initfile(inputStream, parse.getMp3InputData(), frameSkip) == (-1)) {
				throw new RuntimeException("Error reading headers in mp3 input");
			}
		...
		} else if(parse.getInputFormat() == SoundFileFormat.RAW) {
			/* assume raw PCM */
			if(parse.silent < 10) {
			...
			parse.setInputFormat(parse_file_header(gfp, dataInputStream));
		}
		if(gfp.num_samples == -1) {
			double flen = 0;
			try {
				flen = dataInputStream.available();
	...
	private boolean is_syncword_mp123(final byte[] headerptr) {
		...
		case 0x02:
			/* Layer3 */
			if(parse.getInputFormat() != SoundFileFormat.MP3 && parse.getInputFormat() != SoundFileFormat.MP123) {
				return false;
			}
			parse.setInputFormat(SoundFileFormat.MP3);
			break;
		
		case 0x04:
			/* Layer2 */
			if(parse.getInputFormat() != SoundFileFormat.MP2 && parse.getInputFormat() != SoundFileFormat.MP123) {
				return false;
			}
			parse.setInputFormat(SoundFileFormat.MP2);
			break;
		
		case 0x06:
			/* Layer1 */
			if(parse.getInputFormat() != SoundFileFormat.MP1 && parse.getInputFormat() != SoundFileFormat.MP123) {
				return false;
			}
			parse.setInputFormat(SoundFileFormat.MP1);
			break;
		}
	...
	private int lame_decode_initfile(DataInputStream dataInputStream, final MP3Data mp3data, final FrameSkip enc) {
		...
		if(check_aid(buf)) {
			try {
				dataInputStream.readFully(buf, 0, 2);
			} catch(IOException e) {
				e.printStackTrace();
				return -1; /* failed */
			}
			...
			try {
				dataInputStream.skipBytes(aid_header - 6);
			} catch(IOException e) {
				e.printStackTrace();
				return -1; /* failed */
			}
		...
		while(!is_syncword_mp123(buf)) {
			...
			for(i = 0; i < len - 1; i++)
				buf[i] = buf[i + 1];
			try {
				dataInputStream.readFully(buf, len - 1, 1);
			} catch(IOException e) {
				e.printStackTrace();
				return -1; /* failed */
			}
		...
		if(mp3data.bitrate == 0 && !freeformat) {
			...
			return lame_decode_initfile(dataInputStream, mp3data, enc);
		}
	...
	private int decodeFromStream(DataInputStream dataInputStream, float[] pcmLeft, float[] pcmRight, MP3Data mp3data) {
		...
		while(true) {
			try {
				len = dataInputStream.read(buf, 0, 1024);
			} catch(IOException e) {
				e.printStackTrace();
				return -1;
			}
	...
	private boolean is_mpeg_file_format(final SoundFileFormat input_file_format) {
	...
	private int read32BitsLowHigh(DataInputStream dataInputStream) {
		int first = 0xffff & read16BitsLowHigh(dataInputStream);
		int second = 0xffff & read16BitsLowHigh(dataInputStream);
		return (second << 16) + first;
	}
	
	private int read16BitsLowHigh(DataInputStream dataInputStream) {
		try {
			int first = 0xff & dataInputStream.read();
			int second = 0xff & dataInputStream.read();
	...
	private int read16BitsHighLow(DataInputStream dataInputStream) {
		try {
	...
	private int read32BitsHighLow(DataInputStream dataInputStream) {
		int first = 0xffff & read16BitsHighLow(dataInputStream);
		int second = 0xffff & read16BitsHighLow(dataInputStream);
	...
	private double readIeeeExtendedHighLow(DataInputStream dataInputStream) throws IOException {
