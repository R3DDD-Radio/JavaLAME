...
public class Parse {
	...
	public int parse_args(final LameGlobalFlags gfp, final ArrayList<String> argv, final StringBuilder inPath, final StringBuilder outPath, final String[] nogap_inPath, final NoGap ng) {
			...
			if(token.charAt(tokenPos++) == '-') {
				...
				if(token.charAt(tokenPos) == '-') { /* GNU style */
					...
					if(token.substring(tokenPos).equalsIgnoreCase("resample")) {
						argUsed = 1;
						gfp.setOutSampleRate(resample_rate(Double.parseDouble(nextArg)));
					...
					} else if(token.substring(tokenPos).equalsIgnoreCase("ogginput")) {
						System.err.printf("sorry, vorbis support in LAME is deprecated.\n");
						return -1;
						
					} else if(token.substring(tokenPos).equalsIgnoreCase("phone")) {
						...
						boolean ret = id3_tag(gfp, 'n', TextEncoding.TENC_RAW, nextArg);
						argUsed = 1;
						if(ret) {
					...
					} else if(token.substring(tokenPos).equalsIgnoreCase("genre-list")) {
					...
					} else if(token.substring(tokenPos).equalsIgnoreCase("lowpass")) {
						...
						if(val < 0) {
							...
							gfp.lowpassfreq = (int) (val * (val < 50. ? 1.e3 : 1.e0) + 0.5);
						}
						...
						gfp.lowpassfreq = (int) (val * (val < 16. ? 1.e3 : 1.e0) + 0.5);
						...
						if(val < 0.0) {
							...
							gfp.highpassfreq = (int) (val * (val < 16. ? 1.e3 : 1.e0) + 0.5);
						}
					...
					} else if(token.substring(tokenPos).equalsIgnoreCase("quiet") || token.substring(tokenPos).equalsIgnoreCase("silent")) {
					...
					} else if(token.substring(tokenPos).equalsIgnoreCase("version") || token.substring(tokenPos).equalsIgnoreCase("license")) {
						System.out.println(version.getVersion());
						System.out.println();
						usage.printLicense(System.out);
						return -2;
						
					} else if(token.substring(tokenPos).equalsIgnoreCase("help") || token.substring(tokenPos).equalsIgnoreCase("usage")) {
						System.out.println(version.getVersion());
						System.out.println();
						usage.printShort(gfp, System.out);
						return -2;
						
					} else if(token.substring(tokenPos).equalsIgnoreCase("longhelp")) {
						System.out.println(version.getVersion());
						System.out.println();
					...
					} else if(token.substring(tokenPos).equalsIgnoreCase("preset") || token.substring(tokenPos).equalsIgnoreCase("alt-preset")) {
						argUsed = 1;
						{
							int fast = 0, cbr = 0;
					...
					} else if(INTERNAL_OPTS && token.substring(tokenPos).equalsIgnoreCase("tune")) { /* without helptext */
						argUsed = 1;
						{
					...
					} else if(INTERNAL_OPTS && token.substring(tokenPos).equalsIgnoreCase("maskingadjust")) { /* without helptext */
						argUsed = 1;
						gfp.maskingadjust = (float) Double.parseDouble(nextArg);
						
					} else if(INTERNAL_OPTS && token.substring(tokenPos).equalsIgnoreCase("maskingadjustshort")) { /* without helptext */
						argUsed = 1;
						gfp.maskingadjust_short = (float) Double.parseDouble(nextArg);
						
					} else if(INTERNAL_OPTS && token.substring(tokenPos).equalsIgnoreCase("athcurve")) { /* without helptext */
						argUsed = 1;
						gfp.ATHcurve = (float) Double.parseDouble(nextArg);
						
					} else if(INTERNAL_OPTS && token.substring(tokenPos).equalsIgnoreCase("no-preset-tune")) { /* without helptext */
					
					} else if(INTERNAL_OPTS && token.substring(tokenPos).equalsIgnoreCase("substep")) {
						argUsed = 1;
						gfp.internal_flags.substep_shaping = Integer.valueOf(nextArg);
						
					} else if(INTERNAL_OPTS && token.substring(tokenPos).equalsIgnoreCase("sbgain")) { /* without helptext */
						argUsed = 1;
						gfp.internal_flags.subblock_gain = Integer.valueOf(nextArg);
						
					} else if(INTERNAL_OPTS && token.substring(tokenPos).equalsIgnoreCase("sfscale")) { /* without helptext */
					...
					while(tokenPos < token.length()) {
						...
						case 'V':
							...
							gfp.VBR_q_frac = (float) Double.parseDouble(arg) - gfp.getVBRQuality();
							break;
						case 'v':
							/* to change VBR default look in lame.h */
							if(gfp.getVBR() == VbrMode.vbr_off)
						...
						case 's':
							...
							gfp.setInSampleRate((int) (val * (val <= 192 ? 1.e3 : 1.e0) + 0.5));
							break;
						case 'b':
							argUsed = 1;
